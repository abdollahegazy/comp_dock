...
def run_docking(s, uid, csid, conf, counter):
    target_dir = Path(f"../docking/{s}/{uid}/{csid}")
    log_file = target_dir / f"out_c{conf}_p{counter}.log"
    
    # if not log_file.exists():
    #     # Generate ligand structure using VMD
    #     run_command(f"vmd -dispdev text -e {target_dir}/ligand_c{conf}_p{counter}.tcl")
    ...
    #         f"--pH 7.4 "
    #         f"-o {target_dir}/ligand_c{conf}_p{counter}.pdbqt && "
    #         f"conda deactivate"
    #     )
                        ...
                        with centers_file.open() as f:
                            for line in f:
                                ...
                                if not log_file.exists():
                                    ...
                                    run_docking(s, uid, csid, conf, counter)
                                counter += 1

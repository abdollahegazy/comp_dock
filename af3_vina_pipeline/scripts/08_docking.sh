...
do
    ...
    do
        ...
        for conf in {0..11}
        do
            file=../Simulations/$s/pockets/$uid/protein_conf$conf/centers.txt
            if test -f $file; then
                if [ ! -f ../Simulations/$s/docking/$uid/protein_c$conf.pdbqt ]; then
                    conda activate adfr
                    prepare_receptor -r ../Data/$s/$uid/protein_conf$conf.pdb \
                    -o ../Simulations/$s/docking/$uid/protein_c$conf.pdbqt
                    conda deactivate
                fi
                # We need to loop through the list of chemspider compounds
                for csid in $molList
                do
                    mkdir -p ../Simulations/$s/docking/$uid/$csid
                    counter=0 
                    
                    to_do=""
                    while IFS=" *" read -r bx by bz
                    do
                        if [ ! -f ../Simulations/$s/docking/$uid/$csid/out_c${conf}_p${counter}.log ]; then
                        echo "$bx $by $bz"
cat > ../Simulations/$s/docking/$uid/$csid/protein_c${conf}_p${counter}.py <<END
